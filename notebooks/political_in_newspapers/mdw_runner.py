...
@click.option('--min-df', default=None, help='Filter out tokens if DF fraction < ``min_df``.', type=click.FloatRange(0.0, 1.0))
@click.option('--max-df', default=None, help='Filter out tokens if DF fraction > ``max_df``.', type=click.FloatRange(0.0, 1.0))
...
@click.option('--top-n-terms', default=250, help='The total number of most discriminating terms to return for each group', type=int)
@click.option("--group", '-g', multiple=True, metavar="PUBS YEAR-FROM YEAR-TO", help="A group e.g --group AB,DN 1945 1950.", type=click.Tuple([str, click.IntRange(1945, 1989), click.IntRange(1945, 1989)]))
def mdw_run(corpus_folder, max_n_terms, min_df, max_df, min_abs_df, max_abs_df, top_n_terms, group):
    ...
    df = compute_most_discriminating_terms(
        ...
        group1_indices=mdw_gui.group_indicies(v_corpus.document_index, period1, pubs_ids1),
        group2_indices=mdw_gui.group_indicies(v_corpus.document_index, period2, pubs_ids2)
    )
