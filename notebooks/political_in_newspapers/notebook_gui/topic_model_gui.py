    ...
    def prepare_widgets(self):
        ...
        gui = types.SimpleNamespace(
            ...
            n_topics=ipywidgets.IntSlider(
                description='Topics', min=2, max=100, value=20, step=1, layout=ipywidgets.Layout(width='240px')
            ),
...
class TextacyCorpusUserInterface(ComputeTopicModelUserInterface):
    ...
    def prepare_textacy_widgets(self):
        ...
        gui = types.SimpleNamespace(
            ...
            min_freq=ipywidgets.Dropdown(
                description='Min word freq',
                options=list(range(0, 11)),
                value=2,
                layout=ipywidgets.Layout(width='200px', **item_layout),
            ),
            max_doc_freq=ipywidgets.Dropdown(
                description='Min doc %',
                options=list(range(75, 101)),
                value=100,
                layout=ipywidgets.Layout(width='200px', **item_layout),
            ),
            ngrams=ipywidgets.Dropdown(
                description='n-grams', options=ngrams_options, value=[1], layout=ipywidgets.Layout(width='200px')
            ),
            normalize=ipywidgets.Dropdown(
                description='Normalize',
                options=normalize_options,
                value='lemma',
                layout=ipywidgets.Layout(width='200px'),
            ),
            filter_stops=ipywidgets.ToggleButton(
                value=True, description='Remove stopword', tooltip='Filter out stopwords', icon='check'
            ),
            named_entities=ipywidgets.ToggleButton(
                value=False, description='Merge entities', tooltip='Merge entities', icon='check', disabled=False
            ),
            substitute_terms=ipywidgets.ToggleButton(
                value=False, description='Map words', tooltip='Substitute words', icon='check'
            ),
            include_pos=ipywidgets.SelectMultiple(
                options=pos_options,
                value=default_include_pos,
                rows=7,
                layout=ipywidgets.Layout(width='60px', **item_layout),
            ),
            stop_words=ipywidgets.SelectMultiple(
                options=frequent_words, value=list([]), rows=7, layout=ipywidgets.Layout(width='120px', **item_layout)
            ),
        )
        boxes = [
            ipywidgets.VBox(
            ...
            ipywidgets.HBox(
                [ipywidgets.Label(value='POS', layout=ipywidgets.Layout(width='40px')), gui.include_pos],
                layout=ipywidgets.Layout(margin='0px 0px 0px 10px'),
            ),
            ipywidgets.HBox(
                [ipywidgets.Label(value='STOP'), gui.stop_words], layout=ipywidgets.Layout(margin='0px 0px 0px 10px')
            ),
        ]
        return gui, boxes
...
class PreparedCorpusUserInterface(ComputeTopicModelUserInterface):
    ...
    def prepare_source_widgets(self):
        ...
        gui = types.SimpleNamespace(
            filepath=utility.widgets.dropdown(
                description='Corpus', options=corpus_files, value=None, layout=ipywidgets.Layout(width='500px')
            )
        )
