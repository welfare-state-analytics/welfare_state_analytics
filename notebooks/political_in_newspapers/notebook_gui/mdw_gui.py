...
def display_gui(v_corpus, v_documents):
    ...
    gui = types.SimpleNamespace(
        ...
        top_n_terms=ipywidgets.IntSlider(description='#terms', min=10, max=1000, value=200, tooltip='The total number of tokens to return for each group'),
        max_n_terms=ipywidgets.IntSlider(description='#top', min=1, max=2000, value=100, tooltip='Only consider tokens whose DF is within the top # terms out of all terms'),
        min_df=ipywidgets.FloatSlider(description='Min DF%', min=0.0, max=100.0, value=3.0, step=0.1, layout=lw('250px')),
        max_df=ipywidgets.FloatSlider(description='Max DF%', min=0.0, max=100.0, value=95.0, step=0.1, layout=lw('250px')),
        period1=ipywidgets.IntRangeSlider(description='Period', min=year_span[0], max=year_span[1], value=(v_documents.year.min(), v_documents.year.min()+4), layout=lw('250px')),
        period2=ipywidgets.IntRangeSlider(description='Period', min=year_span[0], max=year_span[1], value=(v_documents.year.max()-4, v_documents.year.max()), layout=lw('250px')),
        publication_ids1=ipywidgets.SelectMultiple(description='Publication', options=publications, rows=4, value=(1,), layout=ipywidgets.Layout(width="250px")),
        publication_ids2=ipywidgets.SelectMultiple(description='Publication', options=publications, rows=4, value=(3,), layout=ipywidgets.Layout(width="250px")),
    ...
    def compute_callback_handler(*_args):
        ...
        with gui.output:
            try:
                ...
                x_corpus = v_corpus.slice_by_df(max_df=gui.max_df.value / 100.0, min_df=gui.min_df.value / 100.0, max_n_terms=gui.max_n_terms.value)
                ...
                df = compute_most_discriminating_terms(
                    ...
                    group1_indices=group_indicies(x_corpus.document_index, gui.period1.value, gui.publication_ids1.value),
                    group2_indices=group_indicies(x_corpus.document_index, gui.period2.value, gui.publication_ids2.value)
                )
                if df is not None:
                    display(df)
                else:
