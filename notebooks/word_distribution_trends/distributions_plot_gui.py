...
def display_gui(x_corpus, tokens, n_columns=3):
    ...
    forward = ipywidgets.Button(
        description=">>", button_style='Success', layout=ipywidgets.Layout(width='40px', color='green')
    )
    ...
    display(
        ipywidgets.VBox(
            [progress, ipywidgets.HBox([back, forward, n_count, split]), ipywidgets.HBox([wtokens, output])]
        )
    )
    update_plot()
