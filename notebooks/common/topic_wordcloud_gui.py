...
def display_wordcloud(
    ...
    try:
        ...
        tokens = topic_modelling.get_topic_title(
            topic_token_weights, topic_id, n_tokens=n_words
        )
        ...
        if output_format == "Wordcloud":
            plot_utility.plot_wordcloud(
                df, token="token", weight="weight", max_words=n_words, **opts
            )
        else:
            df = topic_modelling.get_topic_tokens(
                topic_token_weights, topic_id=topic_id, n_tokens=n_words
            )
...
def display_gui(state: TopicModelContainer):
    ...
    gui = types.SimpleNamespace(
        ...
        text_id=text_id,
        text=widgets.HTML(
            value=f"<span class='{text_id}'></span>", placeholder="", description=""
        ),
        topic_id=widgets.IntSlider(
            description="Topic ID",
            min=0,
            max=state.num_topics - 1,
            step=1,
            value=0,
        ...
        progress=widgets.IntProgress(
            min=0, max=4, step=1, value=0, layout=widgets.Layout(width="95%")
        ),
        prev_topic_id=None,
        next_topic_id=None,
    )

    gui.prev_topic_id = widgets_utils.button_with_previous_callback(
        gui, "topic_id", state.num_topics
    )
    gui.next_topic_id = widgets_utils.button_with_next_callback(
        gui, "topic_id", state.num_topics
    )
