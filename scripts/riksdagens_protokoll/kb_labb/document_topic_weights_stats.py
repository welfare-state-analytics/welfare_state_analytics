...
df = pd.read_csv('../data/dtw.csv', index_col=0, sep='\t')

# %%
df_n_docs_per_year = pd.DataFrame(df.groupby('year').document_id.nunique())
df_n_docs_per_year.columns = ['n_total_count']
# %%
df_topic_69 = df[df.topic_id == 68]

# %%
df_topic_69_by_year = df_topic_69.groupby(['year', 'topic_id']).agg({'document_id': 'size', 'weight': 'sum'})
