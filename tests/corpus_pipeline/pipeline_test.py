...
import pandas as pd
from penelope.pipeline import ContentType, CorpusConfig, CorpusPipeline, CheckpointOpts, DocumentPayload
...
def test_pipeline_can_load_pos_tagged_checkpoint():
    ...
    config: CorpusConfig = CorpusConfig.load(path='./tests/test_data/riksdagens-protokoll.yml')
    checkpoint_opts: CheckpointOpts = config.serialize_opts.as_type(ContentType.TAGGED_FRAME)
    pipeline = CorpusPipeline(config=config).load_tagged_frame(
        corpus_filename, checkpoint_opts, extra_reader_opts=config.text_reader_opts
    )
