...
import pandas as pd
from penelope.pipeline import ContentType, CorpusConfig, CorpusPipeline, CheckpointSerializeOpts, DocumentPayload
...
def test_load_corpus_config_returns_correctly_escaped_values():
    ...
    assert config.checkpoint_serialize_opts.sep == '\t'
...
def test_pipeline_can_load_pos_tagged_checkpoint():
    ...
    config: CorpusConfig = CorpusConfig.load(path='./tests/test_data/riksdagens-protokoll.yml')
    options: CheckpointSerializeOpts = config.serialize_opts.as_type(ContentType.TAGGED_FRAME)
