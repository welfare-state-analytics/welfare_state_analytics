...
from notebooks.riksdagens_protokoll.topic_modeling.load_gui import (
    RiksprotLoadGUI,
    probe_paths,
    probe_corpus_version,
    load_metadata,
    probe_vrt_folder,
    probe_document_index,
...
def test_existing_version_file(tmp_path):
    ...
    version = probe_corpus_version(tmp_path)

    assert version == expected_version


def test_existing_config_file():
    ...
    with patch('penelope.pipeline.config.CorpusConfig.load') as mock_load:
        ...
        version = probe_corpus_version(folder)
...
def test_folder_name_with_version_in_config(tmp_path):
    ...
    with patch('penelope.pipeline.config.CorpusConfig.load') as mock_load:
        mock_load.return_value = MagicMock(corpus_version=expected_version)
        version = probe_corpus_version(str(folder))
...
def test_folder_name_with_version_in_folder_name():
    ...
    assert expected_version == probe_corpus_version(f'{expected_version}/apa')
    assert expected_version == probe_corpus_version(f'apa/xyz_{expected_version}')
    assert expected_version == probe_corpus_version(f'apa/xyz_{expected_version}_xyz')
...
@patch('notebooks.riksdagens_protokoll.topic_modeling.load_gui.probe_corpus_version', MagicMock(return_value='v1.2.3'))
@patch(
    'notebooks.riksdagens_protokoll.topic_modeling.load_gui.probe_metadata_filename',
    MagicMock(return_value='metadata_filename'),
)
@patch(
    'notebooks.riksdagens_protokoll.topic_modeling.load_gui.probe_document_index',
    MagicMock(return_value='document_index'),
)
@patch('notebooks.riksdagens_protokoll.topic_modeling.load_gui.probe_vrt_folder', MagicMock(return_value='vrt_folder'))
...
def test_probe_vrt_folder_existing_folder(tmp_path):
    ...
    version = 'v1.2.3'
    folder = tmp_path / version / 'tagged_frames'
    folder.mkdir(parents=True)

    result = probe_vrt_folder(str(tmp_path), version)
...
def test_probe_vrt_folder_no_existing_folder(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        probe_vrt_folder(str(tmp_path), version)
...
def test_probe_document_index_no_existing_file(tmp_path):
    with pytest.raises(FileNotFoundError):
        probe_document_index(str(tmp_path), 'v1')


def test_probe_metadata_filename_existing_file(tmp_path):
    # Create a file in the temporary path
    version = 'v1'
    filename = tmp_path / 'metadata' / f'riksprot_metadata.{version}.db'
    filename.parent.mkdir(parents=True)
    filename.touch()

    result = probe_metadata_filename(str(tmp_path), version)
...
def test_probe_metadata_filename_no_existing_file(tmp_path):
    ...
    with pytest.raises(FileNotFoundError):
        probe_metadata_filename(str(tmp_path), version)
...
def test_riksprot_load_gui_load(mocker):
    ...
    mocker.patch('notebooks.riksdagens_protokoll.topic_modeling.load_gui.load_metadata', return_value={'version': 'v1', 'data_folder': 'data_folder'})

    state: TopicModelContainer = TopicModelContainer()
    gui: RiksprotLoadGUI = RiksprotLoadGUI('data_folder', state)
    gui.load()

    assert gui.state.get('version') == 'v1'
    assert gui.state.get('data_folder') == 'data_folder'


def test_riksprot_load_gui_version():
    state = TopicModelContainer()
    gui = RiksprotLoadGUI('data_folder', state)

    state.set('version', 'v1')
    assert gui.version == 'v1'
